...
def object_detection_and_recognition(ip_address):
    while not exit_event.is_set():
        ...
        if frame is not None:
            ...
            dangerous_objects = {'knife', 'gun', 'scissors'}  # Add more dangerous objects if needed
...
def start_thread(target, ip_address):
    thread = threading.Thread(target=target, args=(ip_address,))
    thread.start()
    return thread
...
def open_operation_window():
    ...
    recognition_button = tk.Button(operation_window, text="Recognition", command=lambda: start_thread(emotion_recognition, ip_entry.get()))
    recognition_button.pack(pady=10)
    detection_button = tk.Button(operation_window, text="Detection", command=lambda: start_thread(object_detection, ip_entry.get()))
    detection_button.pack(pady=10)
    detection_and_recognition_button = tk.Button(operation_window, text="Alert System", command=lambda: start_thread(object_detection_and_recognition, ip_entry.get()))
